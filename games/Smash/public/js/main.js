...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (player2.animationFolder)
        document.getElementById('player2-info-icon').src = `./public/img/icons/${player2.animationFolder}/alt-icon.png`;
    else
...
function getAttckByKeys(player, keys = { top: false, bottom: false }) {
    if (keys.top) {
        player.topAttack(); //attacco alto / overhead
    } else if (keys.bottom) {
        player.sweepAttack(); //attacco basso / sweep
    } else
        player.midAttack();
}
